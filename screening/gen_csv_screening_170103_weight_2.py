...
ratio_arr = np.arange(0,1,0.1)
randindex = np.random.permutation(len(list_2))
list_2_arr = []
pos = math.floor(0.1 * len(list_2))
for i in range(len(ratio_arr)):
    if i == len(ratio_arr)-1:
        randindex_tmp = randindex[pos*i:]
        list_2_arr.append(randindex_tmp)
    else:
        randindex_tmp = randindex[pos * i:pos * (i + 1)]
        list_2_arr.append(randindex_tmp)
...
def cal(list_0, list_1, list_2, list_3, list_4, output, subdir):
    ...
    csv_root = os.path.join(output, '{}'.format(subdir))
...
for i in range(10):
    list_2_index = list_2_arr[0].tolist()
    for j in range(1,i+1):
        list_2_index = list_2_index + list_2_arr[j].tolist()
    list_2_sub = [list_2[i] for i in list_2_index]
    print('====> gen dataset: {}'.format(i))
    cal(list_0, list_1, list_2_sub, list_3, list_4, args.output, '{:.1f}'.format((i+1)/10))