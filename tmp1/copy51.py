...
xls_file = os.path.join(root, '5_1_分级2017.05.01_2017.08.31.xls')
print(xls_file)
...
dst_root_dir = os.path.join(root, 'sort51')
os.makedirs(dst_root_dir, exist_ok=True)
for i in range(5):
    tmp_dir = os.path.join(dst_root_dir, str(i))
    os.makedirs(tmp_dir,exist_ok=True)

src_root_dir = os.path.join(root, '5/1/Save')

nrows = table.nrows     # 表的行，第0行为列的标签


# 第0行为列的标签，因此从第1行开始读取，row[0]不是index，不要混
